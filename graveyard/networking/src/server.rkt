...
(define (op-keepalive game-data id name pwd resp-msg)
  (resp-msg (string-join (list "A" id name pwd) ":")) ; op-keepalive
  (set-update-time game-data id name))

(define (op-update? game-data id name pwd resp-msg c-move-from c-move-to)
  (define s-last-move (hash-ref (hash-ref game-data name) "last-move")) ;stored as (list # #)
  (define c-last-move (list c-move-from c-move-to))
  (if (not (equal? s-last-move c-last-move))
      (resp-msg (string-join (list "D" id name pwd (number->string (car s-last-move)) (number->string (last s-last-move))) ":")) ; op-forward-update
      (resp-msg (string-join (list "A" id name pwd) ":"))) ; op-keepalive
  (set-update-time game-data id name))
...
(define (op-update game-data id name pwd resp-msg move-from move-to)
  ...
  (set-update-time (hash-set game-data name (hash-set game-to-update "last-move" (list move-from move-to))) id name))

(define (op-create game-data id name pwd pieces piece-to-player player-going-first resp-msg)
  ...
  (cond
    ...
    [else (begin (resp-msg (string-join (list "F" id name pwd) ":")) ; op-game-created
                 (hash-set game-data name (hash "pieces" pieces
                                                ...
                                                "player-going-first" player-going-first
...
(define (op-join game-data id name pwd resp-msg)
  (if (null? (hash-ref (hash-ref game-data name) "2-connected"))
      (let ([piece-to-player (hash-ref (hash-ref game-data name) "piece-to-player")]
            ...
            [going-first (number->string (hash-ref (hash-ref game-data name) "player-going-first"))])
        (resp-msg (string-join (list "H" id name pwd pieces piece-to-player going-first) ":")) ; op-forward-join
...
(define (op-joined? game-data id name pwd resp-msg)
  ...
  (cond
    ...
    [(equal? #t is-2-connected?) (let ([piece-to-player (hash-ref (hash-ref game-data name) "piece-to-player")]
                                       ...
                                       [going-first (number->string (hash-ref (hash-ref game-data name) "player-going-first"))])
                                   (resp-msg (string-join (list "H" id name pwd pieces piece-to-player going-first) ":")))] ; op-forward-join
    [(not is-2-connected?) (resp-msg (string-join (list "J" id name pwd) ":"))]) ; op-forward-leave
  (set-update-time game-data id name))
...
(define (exec-opcode game-data code id name pwd msg1 msg2 msg3 resp-msg)
  ...
  (if (or (equal? code "E") (credentials-ok? game-data name pwd)) ; if game is being created, credentials won't be stored yet
      (cond
        ...
        [(equal? code "B") (op-update? game-data id name pwd resp-msg (string->number msg1) (string->number msg2))] 
...
(define (correct-structure client-msg)
  ...
  (let [(regex-match (or (regexp-match-exact? #px"[A | M | O | P]:[1 | 2]:([A-Za-z0-9]){1,20}:([A-Za-z0-9]){1,20}" client-msg) 
                         (regexp-match-exact? #px"[B | C]:[1 | 2]:([A-Za-z0-9]){1,20}:([A-Za-z0-9]){1,20}:[0-8][0-4]:[0-8][0-4]" client-msg)
